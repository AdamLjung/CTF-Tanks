...
class Tank(GamePhysicsObject):
    ...
    def __init__(self, x, y, orientation, sprite, space, tank, framerate):
    ...
    def shoot(self, space):
        ...
        if self.cd < 1:
            ...
            self.cd = self.frame
            return Bullet(self.body.position[0], self.body.position[1], math.degrees(self.body.angle), 3, space, images.bullet, self.id)


class Bullet(GamePhysicsObject):
    def __init__(self, x, y, angle, speed, space, sprite, id):
        super().__init__(x, y, angle, sprite, space, True)
        self.shape.collision_type = 1
        self.max_speed = speed
        self.body.velocity = pymunk.Vec2d(0, speed).rotated(self.body.angle)
        self.shape.parent = self
        self.id = id
